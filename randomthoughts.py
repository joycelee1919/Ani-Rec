...
'...', 'hile', 'let', '",', 'causes', 'certain', 'main', 'first', 'see', 'sees'] # exclude genres
...
def display_image(sorted_df):
    ...
    # Display image according to top genre
...
# Import anime dataset with limited info and extracted keywords
url='https://drive.google.com/file/d/1mFIcE3nnbeDkPbNQa74PGfYGvr7UgaTV/view?usp=sharing'
file_id=url.split('/')[-2]
dwn_url='https://drive.google.com/uc?id=' + file_id
anime_kw = pd.read_csv(dwn_url)

# Limiting ratings didn't work well :( (tried 6)
# What about if we want longer synopsis?
#anime_kw = anime_kw[(anime_kw.Synopsis.str.len() > 50)] #hmm (9214)
anime_kw_syn100 = anime_kw[(anime_kw.Synopsis.str.len() > 100)] # quite good, keep for now (8145)
# Initiate CountVectorizer
cv = CountVectorizer()

def random_thoughts():

    brainstorm = input("What are you thinking right now?")
